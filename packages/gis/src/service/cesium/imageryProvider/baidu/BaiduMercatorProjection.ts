 ...
 * 百度地图
 */
import { MapPoint } from '../type/CommonType';

const EARTH_RADIUS: number = 6370996.81;
...
class BaiduMercatorProjection {
  ...
  getDistanceByMC(point1: MapPoint, point2: MapPoint) {
    if (!point1 || !point2) {
      return 0;
    }
    ...
    let x1: number = this.toRadians(point1['lng']);
    let y1: number = this.toRadians(point1['lat']);
    point2 = this.convertMC2LL(point2);
    if (!point2) {
      return 0;
    }
    let x2: number = this.toRadians(point2['lng']);
    let y2: number = this.toRadians(point2['lat']);
   ...
   */
  getDistanceByLL(point1: MapPoint, point2: MapPoint) {
    if (!point1 || !point2) {
      return 0;
    }
   ...
   */
  convertMC2LL(point: MapPoint): MapPoint {
    if (!point) {
      return { lng: 0, lat: 0 };
    }
    let lnglat: Record<any, any> = {};
    if (this.isWgs84) {
   ...
   */
  convertLL2MC(point: MapPoint): MapPoint {
    if (!point) {
      return { lng: 0, lat: 0 };
    }
    ...
    let temp: MapPoint = { lng: point['lng'], lat: point['lat'] };
    let factor: any = undefined;
    ...
    let mc: MapPoint = this.convertor(point, factor);
    return {
   ...
   * @returns {{lng: *, lat: *}}
   */
  convertor(fromPoint: MapPoint, factor: number[]): MapPoint {
    if (!fromPoint || !factor) {
      return { lng: 0, lat: 0 };
    }
    let x: number = factor[0] + factor[1] * Math.abs(fromPoint['lng']);
    let temp: number = Math.abs(fromPoint['lat']) / factor[9];
    let y: number =
      factor[2] +
      factor[3] * temp +
      factor[4] * temp * temp +
   ...
   * @returns {{lng: number, lat: number}|*}
   */
  lngLatToMercator(point: MapPoint): MapPoint {
   ...
   */
  lngLatToPoint<T>(point: any): Record<string, any> {
    let mercator: MapPoint = this.convertLL2MC(point);
    return {
      x: mercator['lng'],
      y: mercator['lat'],
    };
   ...
   * @returns Point 经纬度
   */
  mercatorToLngLat<T>(point: any): MapPoint {
   ...
   * @returns Point 球面坐标
   */
  pointToLngLat<T>(point: Record<any, any>): MapPoint {
    let mercator: MapPoint = { lng: point.x, lat: point.y };
   ...
   */
  pointToPixel(point: MapPoint, zoom: number, mapCenter: MapPoint, mapSize: Record<any, any>) {
    if (!point) {
      return;
    }
    point = this.lngLatToMercator(point);
    let zoomUnits: number = this.getZoomUnits(zoom);
    let x: number = Math.round((point['lng'] - mapCenter['lng']) / zoomUnits + mapSize.width / 2);
    let y: number = Math.round((mapCenter['lat'] - point['lat']) / zoomUnits + mapSize.height / 2);
    return { x, y };
  }
   ...
   * @param mapSize 地图容器大小
   */
  pixelToPoint(pixel: Record<string, any>, zoom: number, mapCenter: MapPoint, mapSize: Record<any, any>) {
    if (!pixel) {
      return;
    }
    let zoomUnits: number = this.getZoomUnits(zoom);
    let lng: number = mapCenter['lng'] + zoomUnits * (pixel.x - mapSize.width / 2);
    let lat: number = mapCenter['lat'] - zoomUnits * (pixel.y - mapSize.height / 2);
    let point: MapPoint = { lng, lat };
